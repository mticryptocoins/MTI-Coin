...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMTIAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMTIAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMTIAmount = strAnonymizeMTIAmount.remove(strAnonymizeMTIAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMTIAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMTIAmount*COIN) nMaxToAnonymize = nAnonymizeMTIAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMTIAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMTIAmount));
        strAnonymizeMTIAmount = strAnonymizeMTIAmount.remove(strAnonymizeMTIAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMTIAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
