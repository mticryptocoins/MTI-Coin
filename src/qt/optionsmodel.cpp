...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMTIAmount"))
        settings.setValue("nAnonymizeMTIAmount", 1000);
    nAnonymizeMTIAmount = settings.value("nAnonymizeMTIAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMTIAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeMTIAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMTIAmount:
            return QVariant(nAnonymizeMTIAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMTIAmount:
            nAnonymizeMTIAmount = value.toInt();
            settings.setValue("nAnonymizeMTIAmount", nAnonymizeMTIAmount);
            emit AnonymizeMTIAmountChanged(nAnonymizeMTIAmount);
            break;
        default:
            break;
        }
