...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MTI
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MTI
    // Mac: ~/Library/Application Support/MTI
    // Unix: ~/.MTI
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MTI";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MTI.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MTId.pid"));
